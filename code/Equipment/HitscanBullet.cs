...
public sealed class HitscanBullet : Component
{
    ...
    {
        if (FireMode != FireMode.Automatic && !Input.Pressed("Attack1"))
            return false;

        if (FireRate != 0f && TimeSinceFire < 1f / FireRate)
            return false;

        var ammo = Components.Get<Magazine>();

        if (ammo is not null && (ammo.IsReloading || ammo.AmmoInClip <= 0))
            return false;
    ...
    private void ShootBullet()
    {
        ...
        var controller = Components.Get<Pawn>(FindMode.InParent);
    ...
    private void ShootEffects()
    {
        ...
        GameObject.Root.Components.Get<Pawn>()?.Body.Components.Get<SkinnedModelRenderer>()?.Set("b_attack", true);
    }
